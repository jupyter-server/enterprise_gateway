...
The following installs the sample `python_distributed` kernel specification relative to the 3.3.0 release on the given node. This step must be repeated for each node and each kernel specification.
...
tar -zxvf jupyter_enterprise_gateway_kernelspecs-3.3.0.tar.gz --strip 1 --directory $KERNELS_FOLDER/python_distributed/ python_distributed/
```
