...
kernelspec_json = {'argv': ['cat', '{connection_file}'], 'display_name': 'Test kernel: {kernel_name}', }
...
other_kernelspec_location = pytest.fixture(lambda env_jupyter_path: mkdir(env_jupyter_path, "kernels"))
...
def kernel_spec_cache(is_enabled, kernel_spec_manager):
    kspec_cache = KernelSpecCache.instance(kernel_spec_manager=kernel_spec_manager, cache_enabled=is_enabled)
    yield kspec_cache
    kspec_cache = None
...
async def tests_add_spec(kernel_spec_cache, kernelspec_location, other_kernelspec_location):
    ...
    assert str(other_kernelspec_location) in kernel_spec_cache.observed_dirs \
        if kernel_spec_cache.cache_enabled else True
